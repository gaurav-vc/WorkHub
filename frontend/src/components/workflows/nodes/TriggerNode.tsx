...
const iconMap: Record<string, React.ReactNode> = {
  "check-square": <CheckSquare className="h-5 w-5 text-teal-600" />,
  clock: <Clock className="h-5 w-5 text-teal-600" />,
  users: <Users className="h-5 w-5 text-teal-600" />,
  zap: <Zap className="h-5 w-5 text-teal-600" />,
  mail: <Mail className="h-5 w-5 text-teal-600" />,
  bell: <Bell className="h-5 w-5 text-teal-600" />,
  settings: <Settings className="h-5 w-5 text-teal-600" />,
};
    ...
    <div className="flex flex-col items-center group relative">
      ...
      <div className={`bg-white border-2 ${data.execution ? (data.execution.status === 'Success' ? 'border-green-500 shadow-green-100' : 'border-red-500 shadow-red-100') : 'border-slate-200'} rounded-xl w-24 h-24 flex flex-col items-center justify-center shadow-sm group-hover:border-teal-400 group-hover:shadow-md transition-all relative`}>
        ...
        <div className="w-10 h-10 rounded-lg bg-teal-100 flex items-center justify-center mb-2">
          {iconMap[data.icon] || <Zap className="h-5 w-5 text-teal-600" />}
        </div>
